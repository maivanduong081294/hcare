 ...
 * @author: nghidv
 */
class User_Admin_ReportController extends Zend_Controller_Action {
    private $_identity;
    ...
    public function lechMoneyAction() {
		...
		$list = Business_Addon_Report::getInstance()->getLechMoney($month, $year);
		$this->view->list = $list; 
    ...
    $list_hnammobile        = Business_Addon_Options::getInstance()->getCatedHnam();
        $this->view->list_hnammobile = $list_hnammobile;
    $list_ctkm = Business_Addon_Options::getInstance()->getidghn();
    $this->view->list_ctkm = $list_ctkm;
        ...
        if($productsid>0){
            $menu =  Business_Addon_Options::getInstance()->getMenuById($productsid);
        }
        $this->view->menu = $menu;
        $cated_id = (int)  $this->_request->getParam("cated_id");
        $this->view->cated_id = $cated_id;
        $this->view->productsid = $productsid;
        $list = Business_Addon_PromotionFastProduct::getInstance()->get_list_san($itemid, $ictkm);
        foreach ($list as $val){
            $array_products_id[] = $val["product_ids"];
            $name_products[$val["product_ids"]] = $val["product_name"];
            $array_price_km[$val["product_ids"]][$val["ictkm"]]= $val["sum"];
        }
        
        if($array_products_id){
            $array_products_id = array_unique($array_products_id);
            $str_products_id = implode(",", $array_products_id);
            $list_price_by_itemid = Business_Addon_ProductsColor::getInstance()->get_list_by_id3($str_products_id);
            foreach ($list_price_by_itemid as $val2){
                $array_price[$val2["itemid"]] = $val2["price"];
            }
            
            if($productsid>0){
                $list2 = Business_Ws_ProductsItem::getInstance()->get_list_by_itemid_productsid_cateid($str_products_id,$productsid,$cated_id);
                $array_products_id = array();
                foreach ($list2 as $val3){
                    $array_products_id[] = $val3["itemid"];
                }
            }
        }
        $this->view->array_products_id = $array_products_id;
        $this->view->name_products = $name_products;
        $this->view->array_price = $array_price;
        $this->view->array_price_km = $array_price_km;
    }
...
//        $list_color = $_product_color->get_list_by_id($products_id);
    ...
    public function overviewChartAction(){
        ...
        if($this->_identity["username"] == "hnam_trungtq" || $this->_identity["username"]=="hnam_toanhq"){
            $this->_helper->viewRenderer('overview-chart');
        }
        $start_end= $this->_request->getParam("start_end");
        if($start_end ==null){
           $start_end = date("F j, Y")." - ".date("F j, Y"); 
        ...
        foreach ($slist as $items){
            $vid = $items["userid"];
            $actived_vote[$vid] = (int)$items["is_actived"];
            $_votename[] = '"'.$items["abbreviation"].'"';
            $phone_cty[$vid] =0;
            $cphone_cty[$vid] =0;
            $phone_hnam[$vid]=0;
            $cphone_hnam[$vid]=0;
...
//        echo '<pre>';var_dump($list);
        foreach($list as $item) {
            ...
            // tong theo $productsid
            $sum_theoproductsid[$productsid] += $item["sum"];
            $total_theoproductsid[$productsid] += $item["total"];
            //phone
        ...
        $this->view->sum_theoproductsid = $sum_theoproductsid;
        $this->view->total_theoproductsid = $total_theoproductsid;
...
//        $this->view->cwatch_old  = json_encode($cwt_old);
        ...
        foreach ($list as $items){
            ...
            if($items["flag"] ==2){
                if($items["type"] ==3){
                    ...
                    if($items["productsid"] ==9){ // Nước hoa
    ...
    public function exportAction(){
        ...
        if($_option->isBGD($idregency) || $idregency == 47){
            ...
            if($this->_identity["username"] == "hnam_mainhp" || $this->_identity["username"] == "hnam_nhuthq" || $this->_identity["username"] == "hnam_hanln" || $this->_identity["username"] == "hnam_huongntg"){
                $dplay3 =1;  
            }
        }
        ...
        $day = date('Ymd');
        header("Content-Disposition: attachment; filename='Tragop$day.txt'");
...
//        echo chr(255) . chr(254) . mb_convert_encoding($strItem, 'UTF-16LE', 'UTF-8');
        ...
        $day = "_".date('Ymd');
        header("Content-Disposition: attachment; filename=Chargecard_$day.txt");
        header("Content-Type: text/plain; charset=utf-8");
        header('Content-Type: charset=utf-8');
...
//        die();
        $day = "_".date('Ymd');
        header("Content-Disposition: attachment; filename=DetailChargecard_$day.txt");
        header("Content-Type: text/plain; charset=utf-8");
        header('Content-Type: charset=utf-8');
...
//        $list_color = Business_Ws_NewsItem::getInstance()->getListByCateId(722);
        ...
        if($list !=null){
                foreach ($list as $items1){
                    ...
                    $items["colorid"] = $_colorid[$items1["product_color"]];
...
//        $this->_helper->viewRenderer->setNoRender(true);
    ...
    public function phuKienAction(){
        ...
        $list_vote                  = $_zwf_user->getListByUname(false);
        foreach ($list_vote as $key=>$__item){
            if($__item["userid"] == 764){
                unset($list_vote[$key]);
            }
            else {
        ...
        $bgd=0;
        if($_option->isBGD($idregency) || $this->_identity["username"]=="hnam_toanhq"){
            $bgd=1;
            $storeid = (int) $this->_request->getParam("storeid");
        }else{
        ...
        $properties = Business_Addon_ProductProperties::getInstance();
        $_productsHelper = Business_HelpersNew_Products::getInstance();

        $listPropertiesGroup = $properties->getPropertiesByPid(157);
        $this->view->accmenu = $listPropertiesGroup;

        foreach ($listPropertiesGroup as $val) {
            $dataID[] = $val['id'];
        }
        foreach ($dataID as $idGroup) {


            if(in_array($idGroup,array(295,302,301))) // tai nghe
            {
                $check        = $properties->getPropertiesByGroup( 159,$idGroup );
                foreach ($check as $key => $value) {
                    $json   = Zend_Json::decode($value['subid']);
                    foreach ($json as $key => $value1) {
                        $data[]=$value1;
                    }
                }
                $listPropertiesLoai   = $properties->getPropertiesByPid(159,implode(',', $data));  // Tính năng
            }

            else{
                $check        = $properties->getPropertiesByGroup( 158,$idGroup );
                foreach ($check as $key => $value) {
                    $json   = Zend_Json::decode($value['subid']);
                    foreach ($json as $key => $value1) {
                        $data[]=$value1;
                ...
                $listPropertiesLoai   = $properties->getPropertiesByPid(158,implode(',', $data)); // Loại
            }



            $_productsHelper->propetiesHelp($idGroup,$listPropertiesLoai);
            $accmenusub[$idGroup]  = $listPropertiesLoai;
        }
        $this->view->accmenusub = $accmenusub;

        if($option || $menu_id) {
            $subid = $option?$option:$menu_id;
            $list = $properties->getProductsProperties($subid,$start,$end);
            $newList = array();
            foreach($list as $key=>$item) {
                $storeid                   = $item["vote_id"];
                $_itemid                   = $item["products_id"];
                $newList[$_itemid]         = $item;
                $ret[$storeid][$_itemid]   = $item["sum"];
                $count[$storeid][$_itemid] = $item["total"];
            }
            $this->view->list = $newList;
        }
        else {
    ...
    public function nuocHoaAction(){
        ...
        $iexport = (int)  $this->_request->getParam("iexport");
        $bgd=0;
        if($_option->isBGD($idregency) || $this->_identity["username"]=="hnam_hoangtpk"){
            $bgd=1;
            $storeid = (int) $this->_request->getParam("storeid");
        }else{
            $storeid = $this->_identity["parentid"];
        ...
        if($menu_id) {
            $list = $properties->getNuocHoaProperties($menu_id,$start,$end);
            $newList = array();
            foreach($list as $key=>$item) {
                $storeid                   = $item["vote_id"];
                $_itemid                   = $item["products_id"];
...
//            $cated_id =952;
//            $productsid =9;
//            $menus = $_option->get_name_cated_ispecl(952,'Nước hoa');
//        }
        ...
        $iexport = (int)  $this->_request->getParam("iexport");
        $bgd=0;
        if($_option->isBGD($idregency) || in_array($this->_identity["username"],array('hnam_toanhq','hnam_trungtq'))){
            $bgd=1;
            $storeid = (int) $this->_request->getParam("storeid");
        }else{
            $storeid = $this->_identity["parentid"];
        ...
        if($cated_id ==0){// tất cả
            ...
            $list                    = $u_product->getListSalesCatedId($start, $end,$flag,0,$productsid,$is_apple,$is_type,$type_ghn);
            foreach ($list as $items){
                $__cid[] = $items["cated_id"];
            }
            $str_catedid = array_unique($__cid);
           ...
           $slist                    = $u_product->getListSalesProductId($start, $end,$flag,$vonline = 0,$productsid,$cated_id,$is_apple,$is_type,$type_ghn); 
           foreach ($slist as $items){
    ...
    public function salesSaleonlineAction(){
        if ($_REQUEST["d"]==1){die(".");}
        $u_product                  = Business_Addon_UsersProducts::getInstance();
        $_zwf_user                  = Business_Common_Users::getInstance();
        $_option                    = Business_Addon_Options::getInstance();
...
//        }
        ...
        $list                    = $u_product->getListSalesSaleonline($start, $end,$flag,$storeid ,$productsid,$is_apple,$is_type,$type_ghn);
        foreach ($list as $items){
            $__cid[] = $items["cated_id"];
        }
        $str_catedid = array_unique($__cid);
        ...
        foreach($list as $item) {
            $vote_id                    = $item["vote_id"];
            $_cateid                   = $item["cated_id"];
            $ret[$_cateid]   = $item["sum"];
            $count[$_cateid] = $item["total"];
        }
        $col = 100/($count_col + 2);
    ...
    public function exportdanhmucxuathoadonAction(){
        ...
        $flag = (int)  $this->_request->getParam("flag");
        if($productsid >0){
            $list_export_by_cateid = $u_product->get_list_by_productsid($productsid,$start, $end,$flag);
            foreach ($menus as $m){
                $name_cateid[$m["itemid"]] = $m["title"];
            }
            $this->exportbycateidAction($list_export_by_cateid,$name_cateid, $start, $end);
...
//          $__cateid = $name_cateid[$items["cated_id"]];
    ...
    public function salesOrtherAction(){
        ...
        $username = $this->_identity["username"];
         ...
         if($userid==398 || $username=="hnam_thulnt" || $username=="hnam_ngoctg" || $username=="hnam_vinhpt"){
            if($productsid==0 || $productsid==3){
                $productsid =4;
            }else{
                $productsid = (int)$this->_request->getParam("productsid");
            }
        }
        if($productsid == 4 || $productsid ==6 || $productsid==8 || $productsid==9 || $productsid == 10){
            $flag = ""; 
        }
        $this->view->productsid     = $productsid;
        $detail_accept = $_accept_by_cated->getDetailByUserIdProductsId($userid,$productsid);
        $pds = $this->_request->getParam("pdx");
...
//        }
    ...
    public function overviewChartSaleonlineAction(){
        $phone_cty = array();
        $phone_hnam = array();
        $phone_old  = array();
        ...
        $_option                = Business_Addon_Options::getInstance();
        $list_type_ghn = $_option->getTypeGHN();
        foreach ($list_type_ghn as $key=> $val){
            $vid = $key;
            $phone_cty[$vid] =0;
            $cphone_cty[$vid] =0;
            $phone_hnam[$vid]=0;
        ...
        $list_type_ghn3 = implode(",",$list_type_ghn2);
        
        $this->view->list_type_ghn3 = $list_type_ghn3;
        ...
        $_users_products        = Business_Addon_UsersProducts::getInstance();
        $list                   = $_users_products->get_list_ghn_saleonline($start,$end,167);
        // tổng
        $_ptype      = $_option->getCateTargetHnamNew();
        $this->view->ptype = $_ptype;
        foreach ($list as $items){
            if($items["flag"] ==1){ // công ty
                ...
                foreach ($list_type_ghn as $key => $val2){
            ...
            $v_id = $items["type_ghn"];
            if($flag ==1 && $productsid ==3 && $type == 3){
                $phone_cty[$v_id] += $items["sum"];
                $cphone_cty[$v_id] += $items["total"];
                
                // gom VT và GHN
                $phone_cty[41] = $phone_cty[4]+$phone_cty[1];
                $cphone_cty[41] = $cphone_cty[4] +$cphone_cty[1] ;
            }
            if($flag ==2 && $productsid ==3 && $type == 3){
                $phone_hnam[$v_id] += $items["sum"];
                $cphone_hnam[$v_id] += $items["total"];
                
                // gom VT và GHN
                $phone_hnam[41] = $phone_hnam[4]+$phone_hnam[1];
                $cphone_hnam[41] = $cphone_hnam[4] +$cphone_hnam[1] ;
            }
            if($productsid ==3 && $type == 4){
                $phone_old[$v_id] += $items["sum"];
                $cphone_old[$v_id] += $items["total"];
                
                 // gom VT và GHN
                $phone_old[41] = $phone_old[4]+$phone_old[1];
                $cphone_old[41] = $cphone_old[4] +$cphone_old[1] ;
            }
            //tablet
            if($flag ==1 && $productsid ==5 && $type == 3){
                $tablet_cty[$v_id] += $items["sum"];
                $ctablet_cty[$v_id] += $items["total"];
                
                 // gom VT và GHN
                $tablet_cty[41] = $tablet_cty[4]+$tablet_cty[1];
                $ctablet_cty[41] = $ctablet_cty[4] +$ctablet_cty[1] ;
            }
            if($flag ==2 && $productsid ==5 && $type == 3){
                $tablet_hnam[$v_id] += $items["sum"];
                $ctablet_hnam[$v_id] += $items["total"];
                
                // gom VT và GHN
                $tablet_hnam[41] = $tablet_hnam[4]+$tablet_hnam[1];
                $ctablet_hnam[41] = $ctablet_hnam[4] +$ctablet_hnam[1] ;
            }
            if($productsid ==5 && $type == 4){
                $tablet_old[$v_id] += $items["sum"];
                $ctablet_old[$v_id] += $items["total"];
                
                // gom VT và GHN
                $tablet_old[41] = $tablet_old[4]+$tablet_old[1];
                $ctablet_old[41] = $ctablet_old[4] +$ctablet_old[1] ;
            }
            // phụ kiện
            if($flag ==1 && $productsid ==4 && $type == 3){
                $acc_cty[$v_id] += $items["sum"];
                $cacc_cty[$v_id] += $items["total"];
                
                // gom VT và GHN
                $acc_cty[41] = $acc_cty[4]+$acc_cty[1];
                $cacc_cty[41] = $cacc_cty[4] +$cacc_cty[1] ;
            }
            if($flag ==2 && $productsid ==4 && $type == 3){
                $acc_hnam[$v_id] += $items["sum"];
                $cacc_hnam[$v_id] += $items["total"];
                
                // gom VT và GHN
                $acc_hnam[41] = $acc_hnam[4]+$acc_hnam[1];
                $cacc_hnam[41] = $cacc_hnam[4] +$cacc_hnam[1] ;
            }
            // Laptop
            if($flag ==1 && $productsid ==6 && $type == 3){
                $laptop_cty[$v_id] += $items["sum"];
                $claptop_cty[$v_id] += $items["total"];
                
                // gom VT và GHN
                $laptop_cty[41] = $laptop_cty[4]+$laptop_cty[1];
                $claptop_cty[41] = $claptop_cty[4] +$claptop_cty[1] ;
            }
            if($flag ==2 && $productsid ==6 && $type == 3){
                $laptop_hnam[$v_id] += $items["sum"];
                $claptop_hnam[$v_id] += $items["total"];
                
                // gom VT và GHN
                $laptop_hnam[41] = $laptop_hnam[4]+$laptop_hnam[1];
                $claptop_hnam[41] = $claptop_hnam[4] +$claptop_hnam[1] ;
            }
            // Đồng hồ thông minh
            if($flag ==1 && $productsid ==8 && $type == 3){
                $watch_cty[$v_id] += $items["sum"];
                $cwatch_cty[$v_id] += $items["total"];
                
                // gom VT và GHN
                $watch_cty[41] = $watch_cty[4]+$watch_cty[1];
                $cwatch_cty[41] = $cwatch_cty[4] +$cwatch_cty[1] ;
            }
            if($flag ==2 && $productsid ==8 && $type == 3){
                $watch_hnam[$v_id] += $items["sum"];
                $cwatch_hnam[$v_id] += $items["total"];
                
                // gom VT và GHN
                $watch_hnam[41] = $watch_hnam[4]+$watch_hnam[1];
                $cwatch_hnam[41] = $cwatch_hnam[4] +$cwatch_hnam[1] ;
            }
