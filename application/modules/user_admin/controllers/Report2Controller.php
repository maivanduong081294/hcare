 ...
 * @author: nghidv
 */
class User_Admin_Report2Controller extends Zend_Controller_Action {
    private $_identity;
...
//        $this->_helper->viewRenderer->setNoRender(true);
        ...
        $list = $_users_products->getListByCateId(1,$start, $end, $cated_id,"","",$flag);
        $_colorid = array();
//        $list_color = Business_Ws_NewsItem::getInstance()->getListByCateId(722);
        ...
        if($list !=null){
                foreach ($list as &$itemss){
                    $id_addon_user[]    = $itemss["id_addon_user"];
                    $itemss["colorid"] = $_colorid[$itemss["product_color"]];
                    $_products_id[] = $itemss["products_id"];
                 }
...
//        $this->_helper->viewRenderer->setNoRender(true);
        ...
        $list = $_users_products->getListByCateIdLite(1,$start, $end, $cated_id,"","",$flag);
        $day = "_".date('Ymd');
        header("Content-Disposition: attachment; filename=$cated_name$day.csv");
        header("Content-Type: text/csv; charset=utf-8");
        header('Content-Type: charset=utf-8');
