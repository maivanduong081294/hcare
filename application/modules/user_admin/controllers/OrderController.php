...
//        include("/pdf/mpdf60/mpdf.php");
    ...
    public function saveCk2Action(){
        $this->_helper->Layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
        $_orderck = Business_Addon_OrderCk::getInstance();
        $ret = array();
        $arr = array();
//        $ids = $this->_request->getParam("idsk");
        $ids = $this->_request->getParam("ids");
        $data_frm = $this->_request->getParams("data_".$ids);
        $data["ht"] = $data_frm["ht".$ids];
        $data["hd"] = $data_frm["hd".$ids];
        $data["completes"] = $data_frm["completes".$ids];
        $money = $data_frm["money".$ids];
        $__money = str_replace(",", "", $money);
        $data["money"] = $__money;
        $data["note"] = $data_frm["note".$ids];
        $data["note2"] = $data_frm["note2".$ids];
        $data["end_creator"] = $this->_identity["username"];
        $end_datetime = $data_frm["end_datetime".$ids];
        ...
        $data["idhd"] = $data_frm["idhd".$ids];
        $_orderck->update($ids, $data);
//        
        $arr["id"] ="ok";
        $arr["msg"] ="ok";
        $ret[]= $arr;
    ...
    public function editCkAction(){
        ...
        $_vendor = Business_Import_Vendor::getInstance();
        $_option = Business_Addon_Options::getInstance();
        $list_hds = $_option->get_hd();
        $this->view->list_hds = $list_hds;
        
        $list_cks = $_option->get_ck();
        $this->view->list_cks = $list_cks;
        $ack = (int)$this->_request->getParam("ack");
        $this->view->ack = $ack;
        
        $list_vendor = $_vendor->getList();
        foreach ($list_vendor as $v){
            $name_vendor[$v["id"]] = $v["name"];
        }
        $this->view->name_vendor = $name_vendor;
        $p = $this->_request->getParam("p");
        $this->view->list_vendor = $list_vendor;
        $idvendor = (int)$this->_request->getParam("idvendor");
        $this->view->idvendor = $idvendor;
        ...
        $iht           = $this->_request->getParam("iht");
        $this->view->iht = $iht;
...
//        $token = $this->_request->getParam("token");
        $_secKey = "HNAM_ORDER_2016";
        $this->view->skey = $_secKey;
//        $_ztoken = md5($id.$_secKey);
//        if($token != $_ztoken){
//            die('You do not have access.');
//        }
        $idregency = $this->_identity["idregency"];
        $uid = (int)$this->_identity["userid"];
        if($_option->isBGD($idregency) || $idregency ==37 || $idregency ==38){
            $uid = (int)$this->_request->getParam("userid");
        }
        
        $_ck = Business_Addon_OrderCk::getInstance();
        $list = $_ck->get_list($start, $end, $uid, $enabled=1, $idvendor,$iht);
        foreach ($list as $val){
            $__orderid[] = $val["orderid"];
        }
        if($__orderid != NULL){
            $strid = implode(",", $__orderid);
            $slist = $_order->get_list_by_id($strid);
            foreach ($slist as $sl){
                $name_po[$sl["id"]] = $sl["po"];
                $__ack[$sl["id"]] = $sl["ack"];
                $__money_ack[$sl["id"]] = $sl["money_ack"];
            }
        }
        $this->view->name_po = $name_po;
        
        foreach ($list as &$vl){
            $vl["ack"] = $__ack[$vl["orderid"]];
            $vl["money_ack"] = $__money_ack[$vl["orderid"]];
        }
...
//        }
    ...
    public function udelAction(){
        ...
        $query = " update hnam_order_ck set enabled = 0 where orderid = $id";
        Business_Addon_Order::getInstance()->excute($query);
    }
...
//            PO-01-16.03.30.01.05
            ...
            if($supplier_id ==0){
                $err['id'] = "supplier_id";
                $err['msg'] = "Vui lòng lựa chọn nhà cung cấp";
                $ret[] = $err; 
            }
            $ack = (int)  $this->_request->getParam("ack");
            if ($staff_supplier == null) {
                    $err['id'] = "staff_supplier";
                    $err['msg'] = "Vui lòng nhập tên nhân viên";
                    $ret[] = $err;
                }
            if($ack ==0){
                if ($supplier_email == null) {
                    $err['id'] = "supplier_email";
                    $err['msg'] = "Vui lòng nhập địa chỉ email sales nhà cung cấp";
                    $ret[] = $err;
                }
...
//            $ret = array_merge($ret,$_ret);
            ...
            if (count($ret) > 0) {
                ...
                if($id==0){
                    // insert
                    if($ack==1){
                        $money_ack = $this->_request->getParam("money_ack");
                        $money_ack = str_replace(",", "", $money_ack);
                        $data["money_ack"] = (int)$money_ack;
                    }
...
//        }
        ...
        if(count($ret)>0){
            ...
            foreach ($_note as $items){
                $_query[] = "(NULL, '$id', '$now', '$date', '$username', '1', '0', NULL, '$userid', '$supplier_id','$items')";
            }
            ...
            $cc = "";
            $bcc = array("nghi.dang@hnammobile.com","quynh.nguyen@hnammobile.com","duyhuy@hnammobile.com","dongquan@hnammobile.com","tuvan@hnammobile.com","kimngoc.hnam@gmail.com","doandung@hnammobile.com","trungdt.hnam@gmail.com");
