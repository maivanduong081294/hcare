...
class Business_Ws_MenuItem extends Business_Abstract {
    ...
    /**
     * get instance of Business_Ws_MenuItem
     *
     * @return Business_Ws_MenuItem
     */
...
//        $result = FALSE;
    ...
    public function getListByName($menuname, $lang = 1, $ordering = null) {
        ...
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            if ($ordering == null) {
                $query = "SELECT * FROM " . $this->_tablename . " WHERE hide =0 and menuname=? AND lang=? ORDER BY depth asc, myorder asc, itemid";
            } else {
                $query = "SELECT * FROM " . $this->_tablename . " WHERE hide =0 and menuname=? AND lang=? ORDER BY $ordering";
            }
    ...
    public function getListByName2($menuname) {
        $cache = $this->getCacheInstance();

        $key = $this->getKeyList($menuname);
        $result = $cache->getCache($key);
        //$result =FALSE;
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            if ($ordering == null) {
                $query = "SELECT * FROM " . $this->_tablename . " WHERE menuname=? ORDER BY depth asc, myorder asc, itemid";
            } else {
                $query = "SELECT * FROM " . $this->_tablename . " WHERE menuname=? ORDER BY $ordering";
            }
            $data = array($menuname);
            $result = $db->fetchAll($query, $data);
            if (!is_null($result) && is_array($result))
    ...
    public function getSubmenu($cateid) {
        $cache = $this->getCacheInstance();
        ...
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            $query = "SELECT * FROM " . $this->_tablename . " WHERE pid= $cateid ";
            $result = $db->fetchAll($query);
            if (!is_null($result) && is_array($result))
                $cache->setCache($key, $result);
        }
...
//        $result=FALSE;
        if ($result === FALSE) {
            ...
            self::$id_filter = $id;
            if ($depth == null) {
                $list = array_filter($list, "Business_Ws_MenuItem::filterDaisyString");
            } elseif ($id == 0) {
                self::$depth_filter = $depth;
//                $list = array_filter($list, "Business_Ws_MenuItem::filterDepth");
            } else {
                self::$depth_filter = $depth;
                $list = array_filter($list, "Business_Ws_MenuItem::filterDaisyStringAndDepth");
            }
...
//            }
    ...
    public function getDetailById2($id=''){
        $cache = $this->getCacheInstance();
        $key = "getDetailById2".$this->_tablename.$id;
        $result = $cache->getCache($key);
