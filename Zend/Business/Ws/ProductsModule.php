...
class Business_Ws_ProductsModule extends Business_Abstract
{
	private $_tablename = 'ws_productsmodule';
	
	const KEY_LIST = 'productsmodule.list';			//key of list
	const KEY_DETAIL = 'productsmodule.detail.%s';		//key of detail.id
	...
	/**
	 * get instance of Business_Ws_ProductsModule
	 *
	 * @return Business_Ws_ProductsModule
	 */
	...
	public static function mi_buildUrl($lang,$delta,$itemid)
	{
		...
		if($item != null)
		{
			...
			//format : /:lang/:menuitem/news/cate/:productsid/:cateid/title		//for show cate
			//format : /:lang/:menuitem/news/detail/:productsid/:newsitemid/title
			$url = Globals::getBaseUrl() . $lang . '/' . $itemid . '/products/cate/' . $delta . '/' . $menuitemstart . '/' . $title;
		}
		return $url;
	}
	...
	{		
		...
		{
			for($i=0;$i<count($list);$i++) $return[$list[$i]['productsid']] = $list[$i]['name'];
		}
	...
	{
		//:lang/:menuitem/news/detail/:productsid/:newsitemid/title
		$_lang = Business_Ws_Languages::getInstance();
		$mylang = $_lang->getDetail($lang);
		$lang = $mylang['lang'];		
	...
	{		
		$where = array();
		$where[] = "productsid='" . parent::adaptSQL($id) . "'";
		$db = $this->getDbConnection();
