...
class Business_Ws_ProductsItemWh extends Business_Abstract {
    ...
    /**
     * get instance of Business_Ws_ProductsItemWh
     *
     * @return Business_Ws_ProductsItemWh
     */
...
//            var_dump($query);exit();
    ...
    public function getDetailAllTags($id) {
        $db = $this->getDbConnection();
            $query = "SELECT title, itemid,price,original_price,productsid,productscode,cateid FROM ws_productsitem WHERE title != '' and (price > 0 or original_price > 0 ) and onstock = 1 and itemid = $id ORDER BY title ASC";
            $result = $db->fetchAll($query);
...
//        $result = FALSE;
    ...
    public function getListByCate($cateids) {
        ...
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            $query = "SELECT * FROM " . $this->_tablename . " WHERE cateid IN ($cateids) AND enabled = 1 AND onstock = 1 ORDER BY price DESC";
            $data = array();
            $result = $db->fetchAll($query, $data);
            if (!is_null($result) && is_array($result)) {
                $cache->setCache($key, $result);
...
//        $result=false;
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            $query = "SELECT * FROM " . $this->_tablename . " WHERE productsid=? ORDER BY itemid desc LIMIT 0, $limit";
            $data = array($productsid);
            $result = $db->fetchAll($query, $data);
            if (!is_null($result) && is_array($result)) {
                $cache->setCache($key, $result);
...
//                $_typename = ' ORDER BY myorder DESC ';
    ...
    public function getListByProductsID2($strID) {
        $cache = $this->getCacheInstance();
        $strID = trim($strID, ",");
        $key = 'getListByProductsID2'.$this->_tablename . $strID;
        $result = $cache->getCache($key);
        if ($result === FALSE) {
            $db = $this->getDbConnection();
            $query = "SELECT itemid,title,price,original_price FROM " . $this->_tablename . " WHERE itemid IN($strID) ORDER BY posteddate DESC ";
            $data = array();
            $result = $db->fetchAll($query, $data);
