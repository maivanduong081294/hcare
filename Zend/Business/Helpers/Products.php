...
//        }
    ...
    public static function getProductMenu2() {
        $depth = 1;
        $parentid = 0;
...
//        var_dump($leftMenuProducts);exit();
    ...
    public static function getAccMenu($ordering=null) {
        ...
        $parentid = 0;
        $menuname = 'menu_acc';
        $leftMenuAccesories = Business_Helpers_Common::getMenuLev($depth, $parentid, $menuname, $ordering);
        ...
        if (count($leftMenuAccesories) > 0) {
            ...
            foreach ($leftMenuAccesories as &$item) {
                ...
                if ($i++ == count($leftMenuAccesories) - 1)
                    ...
                    $item['class'] = 'class="' . $active . '"';
                $item['link'] = SEOPlugin::getAccesoriesLink($cateid, $title);
                $lev2 = Business_Helpers_Common::getMenuLev($depth = 2, $parentid = $item['itemid'], $menuname, $ordering);
                if (count($lev2) > 0) {
...
//                        pre($item['sub']);
    ...
    public static function getAccMenu2($ordering=null) {
        //get left accesories
        $depth = 1;
        $parentid = 0;
        $menuname = 'menu_acc';
...
//        var_dump($leftMenuAccesories);exit();   
        ...
        if (count($leftMenuAccesories) > 0) {
            ...
            foreach ($leftMenuAccesories as &$item) {
                ...
                //merge title lev2
                if ($item["pid"]>0) {
                    foreach($leftMenuAccesories as $m) {
                        if ($m["itemid"]==$item["pid"]) {
...
//                        pre($item['sub']);
    ...
    public static function getAccMenu3($ordering=null) {
        //get left accesories
        $depth = 1;
        $parentid = 0;
        $menuname = 'menu_acc';
