...
class Business_Addon_Pos extends Business_Abstract
{
	private $_tablename = 'quynhn_pos';
	...
	/**
	 * get instance of Business_Addon_Pos
	 *
	 * @return Business_Addon_Pos
	 */
	public static function getInstance()
	{
		if(self::$_instance == null)
		{
			self::$_instance = new Business_Addon_Pos();
		}
...
//            Business_Addon_Options::getInstance()->flush_cache(5);
       ...
       public function get_list_by_id($strid)
	{
       ...
       public function get_list_by_billid($billid)
	{
            $db = $this->getDbConnection();
            $query = " SELECT * FROM " . $this->_tablename ." WHERE enabled=1 and billid IN ($billid)";
            $result = $db->fetchAll($query);
            return $result;
	}
	public function get_list($q,$agencyid,$start="" ,$end="",$pid_agency=0,$userid=0 )
	{
            $cache = $this->getCacheInstance();
            $key = $this->get_key_list($q.$agencyid.$start.$end.$pid_agency.$userid);
            ...
            if($result === FALSE)
            {
                ...
                if((int)$pid_agency >0){
                    $query .=" and pid_agency  = $pid_agency";
                }
                if($start){
	...
	public function get_group_by_date($date,$agencyid,$pid_agency)
	{
            $cache = $this->getCacheInstance();
            $key = "get_group_by_date".$this->_tablename.$date.$agencyid.$pid_agency;
            $result=$cache->getCache($key);
            if($result === FALSE)
            {
                $db = $this->getDbConnection();
                $query = " SELECT agencyid,pid_agency,count(*) as total,sum(money) as sum FROM " . $this->_tablename ." WHERE enabled=1 and datetime <='$date'";
                if((int)$agencyid >0){
                    $query .=" and agencyid = $agencyid";
                }
                if((int)$pid_agency >0){
                ...
                $query .=" group by agencyid,pid_agency";
                $result = $db->fetchAll($query);
                if(!is_null($result) && is_array($result)){
                    $cache->setCache($key, $result,  $this->_timecache);
                }
