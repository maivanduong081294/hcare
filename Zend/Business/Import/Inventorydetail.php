...
class Business_Import_Inventorydetail extends Business_Abstract
{
	private $_tablename = 'import_inventory_detail';
	...
	public static function getInstance()
	{
		...
		{
			self::$_instance = new Business_Import_Inventorydetail();
		}
	...
	public function getWheres($field_where,$value_where, $id, $field = "*")
	{
            $db = $this->getDbConnection();
            $query = " SELECT ".$field." FROM " . $this->_tablename ." WHERE id <> ".$id." and ".$field_where." = ?";
            $data = array($value_where);
            $result = $db->fetchAll($query,$data);
            if($result != null && is_array($result))
            {
