...
class Business_Import_Viewitems extends Business_Abstract
{
	private $_tablename = 'ws_viewitems';
	...
	public static function getInstance()
	{
		...
		{
			self::$_instance = new Business_Import_Viewitems();
		}
	...
	{		
		$db = Globals::getDbConnection('hnam_log', false);
		return $db;	
	}
	...
	public function getListCount($created_day="", $end_day="",$listitem)
	{
        $db = $this->getDbConnection();
        $query = "select viewdate,itemid,count(itemid) as total from " . $this->_tablename . " where itemid in (".$listitem.") ";
        if($created_day != null){
                $query .=" and viewdate >=  '$created_day'";
            }
            if($end_day != null){
                $query .="  and viewdate <= '$end_day 23:59:59'";
            }
         $query .= "  group by itemid";
        $result = $db->fetchAll($query);
        return $result;
	}
	public function getListCountSum($listitem)
	{
        $db = $this->getDbConnection();
        $query = "select viewdate,itemid,count(itemid) as total from " . $this->_tablename . " where itemid in (".$listitem.") ";
        $query .= "  group by itemid";
