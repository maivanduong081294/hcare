...
class Business_Import_Position extends Business_Abstract
{
	private $_tablename = 'import_position';
	...
	public static function getInstance()
	{
		...
		{
			self::$_instance = new Business_Import_Position();
		}
	...
	{
            $db = $this->getDbConnection();
            $query = " SELECT ".$field." FROM " . $this->_tablename ." WHERE ".$field_where." = ".$value_where."";
			//echo $query;die;
            $result = $db->fetchAll($query);
            if($result != null && is_array($result))
            {
