...
class Business_Import_Billdetail extends Business_Abstract
{
	private $_tablename = 'import_bill_detail';
	...
	/**
	 * get instance of Business_Import_Billdetail
	 *
	 * @return Business_Import_Billdetail
	 */
	public static function getInstance()
	{
		if(self::$_instance == null)
		{
			self::$_instance = new Business_Import_Billdetail();
		}
    ...
    public function getListByIdBill($id_bill, $field = "*")
	{
            $id = intval($id);
            $db = $this->getDbConnection();
            $query = " SELECT ".$field." FROM " . $this->_tablename ." WHERE id_bill = ?";
            $data = array($id_bill);
	...
	public function getWhere4($field1,$valu1,$field2,$valu2,$field3,$valu3,$field4,$valu4, $field = "*")
	{
            $id = intval($id);
            $db = $this->getDbConnection();
            $query = " SELECT ".$field." FROM " . $this->_tablename ." WHERE ".$field1." = ? and ".$field2." = ? and ".$field3." = ? and ".$field4." = ?";
            $data = array($valu1,$valu2,$valu3,$valu4);
            $result = $db->fetchAll($query,$data);
            if($result != null && is_array($result))
            {
                    return $result[0];
