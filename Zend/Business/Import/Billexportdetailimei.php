...
class Business_Import_Billexportdetailimei extends Business_Abstract
{
	private $_tablename = 'import_bill_export_detail_imei';
	...
	/**
	 * get instance of Business_Import_Billexportdetailimei
	 *
	 * @return Business_Import_Billexportdetailimei
	 */
	public static function getInstance()
	{
		if(self::$_instance == null)
		{
			self::$_instance = new Business_Import_Billexportdetailimei();
		}
	...
	public function getWhereRow($field_where,$val_where, $field = "*")
	{
            $id = intval($id);
            $db = $this->getDbConnection();
            $query = " SELECT ".$field." FROM " . $this->_tablename ." WHERE ".$field_where." = ?";
            $data = array($val_where);
            $result = $db->fetchAll($query,$data);
            if($result != null && is_array($result))
            {
                    return $result[0];
